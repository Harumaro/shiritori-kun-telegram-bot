...
const rules = {
  longVowelFallbackToPreviousKanaIsValid: false,
  allowNKana: false,
  smallKanaFallbackToPreviousKanaIsValid: false
};
...
function areWordsShiritoriCompliant (bef, aft, optionalRules = {}) {
  _rules = utils.extend(true, rules, optionalRules);
  ...
  if (befHira.charCodeAt(befHira.length - 1) === aftHira.charCodeAt(0) ||
    ...
    (_rules.smallKanaFallbackToPreviousKanaIsValid && befHira.charCodeAt(befHira.length - 2) === aftHira.charCodeAt(0) && befHira.charCodeAt(befHira.length - 1) === aftHira.charCodeAt(1)))) ||
    (_rules.longVowelFallbackToPreviousKanaIsValid && isLongVowel && befHira.charCodeAt(befHira.length - 2) === aftHira.charCodeAt(0)) ||
    (_rules.allowNKana && endsInNG && ['ん', 'な', 'に', 'ぬ', 'ね', 'の'].indexOf(aftHira[0]))) {
    return true;
  }
