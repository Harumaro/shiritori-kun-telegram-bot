...
class Game extends EventEmitter {
  ...
  _handlePlayerJoin (channel) {
    this.emit(Channel.states.join, this);
  }

  _handlePlayerOwnTurn (channel) {
    this.emit(Channel.states.ownTurn, this);
  }

  _handlePlayerTurnOver (channel) {
    this.emit(Channel.states.turnOver, this);
  }

  _handlePlayerTurnSkipped (channel) {
    this.emit(Channel.states.turnSkipped, this);
  }

  _handlePlayerWrongWord (channel) {
    this.emit(Channel.states.wrongWord, this);
  }

  _handlePlayerLost (channel) {
    this.emit(Channel.states.lost, this);
  }

  _handleGameOver (channel) {
    console.log('game over');
    var queue = channel.getQueue();
    game.leave(queue[0], channel.getId());
  ...
  getChannel (chId) {
    var channel = this._channels[chId] || new Channel(chId);

    channel.on(Channel.states.join, this._handlePlayerJoin.bind(channel));
    channel.on(Channel.states.ownTurn, this._handlePlayerOwnTurn.bind(channel));
    channel.on(Channel.states.turnOver, this._handlePlayerTurnOver.bind(channel));
    channel.on(Channel.states.turnSkipped, this._handlePlayerTurnSkipped.bind(channel));
    channel.on(Channel.states.wrongWord, this._handlePlayerWrongWord.bind(channel));
    channel.on(Channel.states.lost, this._handlePlayerLost);
    channel.on('game_over', this._handleGameOver.bind(channel));

    return this._channels[chId] = channel;
  }
